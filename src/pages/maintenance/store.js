...
import { storeFields } from "../../common/fieldsForm"
        ...
        const getAll = async () => {
            try {
                ...
                const response = await getMainProvide(
                    routesApi.STORE_GET_ALL(localStorage.getItem("sede"))
                )
    ...
    return (
        <React.Fragment>
                ...
                <Container fluid={true}>
                    <Row>
                        <Col lg={12}>
                            <Card>
                                <CardBody>
                                    <CardTitle className="mb-4">Almacen</CardTitle>
                                    <div className="my-3 row">
                                        <div className="col-sm-2">
                                            <ModalFormWithButton
                                                ...
                                                handle={async data => {
                                                    try {
                                                        const response = await postMainProvide(routesApi.STORE_SAVE(), data)
                                                ...
                                                reset={true}
                                                data={{
                                                    fields: storeFields.save(localStorage.getItem("sede")),
                                                }}
                                            />
                                        </div>
                                    ...
                                    {loadingAll ? (
                                        ...
                                        <div className="table-responsive">
                                            <TableEasy
                                                ...
                                                data={data_view}
                                                idx={true}
                                                records={[{ label: "Almacen", key: "name" }]}
                                                action={data => (
                                                    <>
                                                        <Button
                                                            color="success"
                                                        ...
                                                        <ButtonStatus
                                                            url={routesApi.STORE_UPDATE(data.id)}
                                                            id={data.id}
                                                            model={storeFields.update(data)}
                                                            status={data.status}
            ...
            <ModalFormWithoutButton
                ...
                handle={async data => {
                    try {
                        const response = await putMainProvider(
                            routesApi.STORE_UPDATE(selectedRecord.id),
                            data
                        )
                        console.log(response)
                ...
                data={{
                    fields: storeFields.update(selectedRecord),
                }}
            />
        </React.Fragment>
    )
