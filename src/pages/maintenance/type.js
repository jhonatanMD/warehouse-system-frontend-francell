...
import { typeFields } from "../../common/fieldsForm"
  ...
  return (
    <React.Fragment>
        ...
        <Container fluid={true}>
          <Row>
            <Col lg={12}>
              <Card>
                <CardBody>
                  <CardTitle className="mb-4">Tipo</CardTitle>
                  <div className="my-3 row">
                    <div className="col-sm-2">
                      <ModalFormWithButton
                        ...
                        handle={async data => {
                          try {
                            const response = await postMainProvide(routesApi.TYPE_SAVE(), data)
                        ...
                        reset={true}
                        data={{
                          fields: typeFields.save(localStorage.getItem("sede")),
                        }}
                      />
                    </div>
                      ...
                      <TableEasy
                        ...
                        action={data => (
                          <>
                            ...
                            <ButtonStatus
                              url={routesApi.TYPE_UPDATE(data.id)}
                              id={data.id}
                              status={data.status}
      ...
      <ModalFormWithoutButton
        ...
        handle={async data => {
          try {
            const response = await putMainProvider(routesApi.TYPE_UPDATE(selectedRecord.id), data)
            console.log(response)
        ...
        data={{
          fields: typeFields.update(selectedRecord),
        }}
      />
    </React.Fragment>
  )
...
export default withRouter(connect(mapStateToProps, {})(Type))
