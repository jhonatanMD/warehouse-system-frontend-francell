...
//Helpers
import { postMainProvide, getMainProvide, routesApi, axios, putMainProvider } from "../../services/request"
...
const Company = props => {
  ...
  useEffect(() => {
    // EXPERIMENTAL: Funcionalidad en axios para cancelar la carga al momento de recounstruir el componente
    let didCancel = false
    let cancel
    const getAll = async () => {
      try {
  ...
  if (searchText && allRecords.length > 0) {
    data_view = allRecords.filter(e => e[keyFilter] != null).filter(e => e[keyFilter].toUpperCase().includes(searchText.toUpperCase())
    )
  }
        ...
        <Container fluid={true}>
          <Row>
            <Col lg={12}>
              <Card>
                <CardBody>
                  <CardTitle className="mb-4">Empresas</CardTitle>
                  ...
                  {loadingAll ? (
                    ...
                    <div className="table-responsive">
                      <TableEasy
                        ...
                        data={data_view}
                        idx={true}
                        records={[{ label: "Empresa", key: "name" }, { label: "RUC", key: "rut" }]}
                        action={data => (
                          <>
                            <Button
                              color="success"
                            ...
                            {/* <Button
                              color="success"
                              className="btn-rounded waves-effect waves-light btn btn-info btn-sm"
                              type="bottom"
                              onClick={() => history.push(`/company/${data.id}/sede`)}
                            >
                              Sedes
                            </Button> */}
                          </>
