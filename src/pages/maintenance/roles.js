    ...
    const getAll = async () => {
      try {
        ...
        const response = await getMainProvide(
          routesApi.ROLES_GET_ALL(localStorage.getItem("sede"))
          //routesApi.ROLES_GET_ALL()
        )
  ...
  return (
    <React.Fragment>
        ...
        <Container fluid={true}>
          <Row>
            <Col lg={12}>
              <Card>
                <CardBody>
                  <CardTitle className="mb-4">Roles</CardTitle>
                  <div className="my-3 row">
                    <div className="col-sm-2">
                      <ModalFormWithButton
                        ...
                        handle={async data => {
                          try {
                            const response = await postMainProvide(routesApi.ROLES_SAVE(), data)
                    ...
                    {HandleGuard.renderView(
                      <div className="col-sm-2">
                        <ModalFormWithButton
                          ...
                          reset={true}
                          data={{
                            fields: rolesFields.save(localStorage.getItem("sede")),
                          }}
                        />
                      </div>,
                      ["6_insertar"]
                          ...
                          <>
                            ...
                            </Button>
                            {
                              HandleGuard.renderView(
                                ...
                                >
                                  Permisos
                                </Button>, [`${KEY_MODULE}_update`], true
                              )
                            }

                            <ButtonStatus
                              url={routesApi.ROLES_UPDATE(data.id)}
                              id={data.id}
                              status={data.status}
      ...
      <ModalFormWithoutButton
        ...
        handle={async data => {
          try {
            const response = await putMainProvider(routesApi.ROLES_UPDATE(selectedRecord.id), data)
            console.log(response)
