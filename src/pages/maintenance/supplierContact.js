...
import { supplierContactFields } from "../../common/fieldsForm"
import TableEasy from "../../helpers/tableEasy"
import toastr from "toastr"
//Helpers
import { postMainProvide, getMainProvide, putMainProvider, routesApi } from "../../services/request"
import {
  ...
  return (
    <React.Fragment>
        ...
        <Container fluid={true}>
          <Row>
            <Col lg={12}>
              <Card>
                <CardBody>
                  <CardTitle className="mb-4">Contacto del proovedor</CardTitle>
                  <div className="my-3 row">
                    <div className="col-sm-2">
                      <ModalFormWithButton
                        handle={async data => {
                          try {
                            const response = await postMainProvide(routesApi.SUPPLIER_SAVE(), data)
                        ...
                        reset={true}
                        data={{
                          fields: supplierContactFields.save(),
                        }}
                      />
                    </div>
                  ...
                  {loadingAll ? (
                    ...
                    <div className="table-responsive">
                      <TableEasy
                        data={data_view}
                        idx={true}
                        records={[{ label: "Nombre contacto", key: "name" }]}
                        action={data => (
                          <>
                            <Button
                              color="success"
                            ...
                            <ButtonStatus
                              url={routesApi.SUPPLIER_UPDATE(data.id)}
                              id={data.id}
                              status={data.status}
      ...
      <ModalFormWithoutButton
        ...
        handle={async data => {
          try {
            const response = await putMainProvider(routesApi.SUPPLIER_UPDATE(selectedRecord.id), data)
            console.log(response)
        ...
        data={{
          fields: supplierContactFields.update(selectedRecord),
        }}
      />
    </React.Fragment>
  )
...
export default withRouter(connect(mapStateToProps, {})(SupplierContact))
