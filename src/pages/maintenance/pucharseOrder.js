    ...
    const getAll = async () => {
      try {
        ...
        const response = await getMainProvide(
          routesApi.BUY_ORDER_GET_ALL_BY_SEDE(localStorage.getItem("sede"))
        ...
        <Container fluid={true}>
          <Row>
            <Col lg={12}>
              <Card>
                <CardBody>
                  <CardTitle className="mb-4">Orden de compra</CardTitle>
                  ...
                  {loadingAll ? (
                    ...
                    <div className="table-responsive">
                      <TableEasy
                        ...
                        idx={true}
                        records={[
                          { label: "EMPRESA", key: "sresEmpresa" },
                          { label: "RUC PROVEEDOR", key: "rucProveedor" },
                        ...
                        action={data => (
                          <>
                            <Button
                              ...
                              type="bottom"
                              onClick={() => history.push(`/purchase-order-view/${data.id}`)}
                            >
                              Ver
                            </Button>
                            <a
                              className="btn btn-success btn-block btn-sm"
                              href={
                                config.mainApi + routesApi.BUY_ORDER_REPORT() + "?id=" + data.id
                              }
                              target="_blank"
                            >
                              reporte
...
export default withRouter(connect(mapStateToProps, {})(PucharseOrder))
