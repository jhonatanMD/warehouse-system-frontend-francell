...
import { employeeFields } from "../../common/fieldsForm"
    ...
    const getAll = async () => {
      try {
        ...
        const response = await getMainProvide(
          routesApi.EMPLOYEE_GET_ALL()
        )
        console.log(response);
        setAllRecords(
          response
            ? response.map(value => {
              value.display_name = `${value.lastName} ${value.secondSurName} ${value.name}`
              return value
            })
            : []
        )
        ...
        <Container fluid={true}>
          <Row>
            <Col lg={12}>
              <Card>
                <CardBody>
                  <CardTitle className="mb-4">Empleados</CardTitle>
                  <div className="my-3 row">
                    <div className="col-sm-2">
                      <ModalFormWithButton
                        ...
                        handle={async data => {
                          try {
                            const response = await postMainProvide(routesApi.EMPLOYEE_SAVE(), data)
                            toastr.success(MSG_SUCCESSFUL_REGISTRATION)
                            setAllRecords([
                              Object.assign(response, {
                                display_name: `${response.lastName} ${response.secondSurName} ${response.name}`,
                              }),
                              ...allRecords,
                            ])
                          } catch (error) {
                  ...
                  {loadingAll ? (
                    ...
                    <div className="table-responsive">
                      <TableEasy
                        ...
                        data={data_view}
                        idx={true}
                        records={[{ label: "Nombre", key: "display_name" }]}
                        action={data => (
                          <>
                            <Button
                              color="success"
                            ...
                            <ButtonStatus
                              url={routesApi.EMPLOYEE_UPDATE(data.id)}
                              id={data.id}
                              status={data.status}
