...
import { brandFields } from "../../common/fieldsForm";
  ...
  return (
    <React.Fragment>
        ...
        <Container fluid={true}>
          <Row>
            <Col lg={12}>
              <Card>
                <CardBody>
                  <CardTitle className="mb-4">Marca</CardTitle>
                  <div className="my-3 row">
                    <div className="col-sm-2">
                      <ModalFormWithButton
                        ...
                        handle={async data => {
                          try {
                            const response = await postMainProvide(routesApi.BRAND_SAVE(), data)
                      ...
                      <TableEasy
                        ...
                        action={data => (
                          <>
                            ...
                            <ButtonStatus
                              url={routesApi.BRAND_UPDATE(data.id)}
                              id={data.id}
                              status={data.status}
      ...
      <ModalFormWithoutButton
        ...
        handle={async data => {
          try {
            const response = await putMainProvider(routesApi.BRAND_UPDATE(selectedRecord.id), data)
            console.log(response)
        ...
        data={{
          fields: brandFields.update(selectedRecord),
        }}
      />
    </React.Fragment>
  )
