...
import { materialFields } from "../../common/fieldsForm"
  ...
  return (
    <React.Fragment>
        ...
        <Container fluid={true}>
          <Row>
            <Col lg={12}>
              <Card>
                <CardBody>
                  <CardTitle className="mb-4">Calidad</CardTitle>
                  <div className="my-3 row">
                    <div className="col-sm-2">
                      <ModalFormWithButton
                        ...
                        handle={async data => {
                          try {
                            const response = await postMainProvide(routesApi.MATERIAL_SAVE(), data)
                        ...
                        reset={true}
                        data={{
                          fields: materialFields.save(localStorage.getItem("sede")),
                        }}
                      />
                    </div>
                  ...
                  {loadingAll ? (
                    ...
                    <div className="table-responsive">
                      <TableEasy
                        ...
                        data={data_view}
                        idx={true}
                        records={[{ label: "Calidad", key: "name" }]}
                        action={data => (
                          <>
                            <Button
                              color="success"
                            ...
                            <ButtonStatus
                              url={routesApi.MATERIAL_UPDATE(data.id)}
                              id={data.id}
                              status={data.status}
      ...
      <ModalFormWithoutButton
        ...
        handle={async data => {
          try {
            const response = await putMainProvider(
              routesApi.MATERIAL_UPDATE(selectedRecord.id),
              data
            )
            console.log(response)
        ...
        data={{
          fields: materialFields.update(selectedRecord),
        }}
      />
    </React.Fragment>
  )
