...
import { userFields } from "../../common/fieldsForm"
  ...
  return (
    <React.Fragment>
                  ...
                  <div className="my-3 row">
                    <div className="col-sm-2">
                      <ModalFormWithButton
                        ...
                        handle={async data => {
                          try {
                            const response = await postMainProvide(routesApi.USER_SAVE(), data)
                  ...
                  {loadingAll ? (
                    ...
                    <div className="table-responsive">
                      <TableEasy
                        ...
                        data={data_view}
                        idx={true}
                        records={[{ label: "Usuario", key: "user" }]}
                        action={data => (
                          <>
                            <Button
                              color="success"
                            ...
                            <ButtonStatus
                              url={routesApi.USER_UPDATE(data.id)}
                              id={data.id}
                              status={data.status}
                              allRecords={allRecords}
                              model={userFields.update(data)}
      ...
      <ModalFormWithoutButton
        ...
        handle={async data => {
          try {
            const response = await putMainProvider(
              routesApi.USER_UPDATE(selectedRecord.id),
              data
            )
            console.log(response)
        ...
        data={{
          fields: userFields.update(selectedRecord),
        }}
      />
    </React.Fragment>
  )
