...
import { headQuarterFields } from "../../common/fieldsForm"
    ...
    const getAll = async () => {
      try {
        ...
        const response = await getMainProvide(
          routesApi.HEAD_QUARTER_GET_ALL()
        )
  ...
  return (
    <React.Fragment>
                  ...
                  <div className="my-3 row">
                    <div className="col-sm-2">
                      <ModalFormWithButton
                        ...
                        handle={async data => {
                          try {
                            const response = await postMainProvide(routesApi.HEAD_QUARTER_SAVE(), data)
                        ...
                        reset={true}
                        data={{
                          fields: headQuarterFields.save(localStorage.getItem("sede")),
                        }}
                      />
                    </div>
                      ...
                      <TableEasy
                        ...
                        action={data => (
                          <>
                            ...
                            {/* EN LOS REGISTOS NO TRAE STATUS */}
                            {/* <ButtonStatus
                              url={routesApi.HEAD_QUARTER_UPDATE(data.id)}
                              id={data.id}
                              status={data.status}
      ...
      <ModalFormWithoutButton
        ...
        handle={async data => {
          try {
            const response = await putMainProvider(routesApi.HEAD_QUARTER_UPDATE(selectedRecord.id), data)
            console.log(response)
        ...
        data={{
          fields: headQuarterFields.update(selectedRecord),
        }}
      />
    </React.Fragment>
  )
...
export default withRouter(connect(mapStateToProps, {})(HeadQuarter))
