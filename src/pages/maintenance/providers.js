...
import { providersFields } from "../../common/fieldsForm"
import TableEasy from "../../helpers/tableEasy"
import Guard from "helpers/guard";
import toastr from "toastr"
//Helpers
import { postMainProvide, getMainProvide, routesApi, putMainProvider } from "../../services/request"
import {
  ...
  return (
    <React.Fragment>
        ...
        <Container fluid={true}>
          <Row>
            <Col lg={12}>
              <Card>
                <CardBody>
                  <CardTitle className="mb-4">Proovedores</CardTitle>
                  <div className="my-3 row">
                    <div className="col-sm-2">
                      <ModalFormWithButton
                        ...
                        handle={async data => {
                          try {
                            const response = await postMainProvide(routesApi.PROVIDER_SAVE(), data)
                      ...
                      <TableEasy
                        ...
                        action={data => (
                          <>
                            ...
                            <ButtonStatus
                              url={routesApi.PROVIDER_UPDATE(data.id)}
                              id={data.id}
                              status={data.status}
                              model={providersFields.update(data)}
      ...
      <ModalFormWithoutButton
        ...
        handle={async data => {
          try {
            const response = await putMainProvider(routesApi.PROVIDER_UPDATE(selectedRecord.id), data)
            console.log(response)
        ...
        data={{
          fields: providersFields.update(selectedRecord),
        }}
      />
    </React.Fragment>
  )
